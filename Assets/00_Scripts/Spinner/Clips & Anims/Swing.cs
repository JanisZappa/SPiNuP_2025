...
namespace Clips
{
    ...
    //  Used in StickSwing_Anim  //
        ...
        //  Solo Bounds  //
            ...
            {
                Item warpBuddy = (Item) startStick.Item.GetLink(linkType.Warp, true).b;
                if (warpBuddy != null)
                {
                    bool warpHappens = runTime >= Warp.warpTime;
        ...
        //  Track Bounds  //
            {
                Item  item  = startStick.Item;
                Track track = item.parent;
                ...
                int subStep  = track.SubStep(item, start);
                int subIndex = track.SubIndex(subStep);

                float boundStart = start;
                ClipBounds cBounds = null;

                float radiusTweak = startStick.Item.radius - track.maxItemRadius + spinner.size.y;
                
                for (float timeA = start; timeA <= end; timeA += ClipBounds.SearchStep)
                {
                    int newSubStep = track.SubStep(item, timeA);
                    int diff = newSubStep - subStep;
                    int absDiff = Mathf.Abs(diff);
                    
                    if (absDiff == 0)
                        continue;

                    int newSubIndex = track.SubIndex(newSubStep);
                    ...
                    if (newSubIndex != subIndex)
                    {
                        ...
                        if (absDiff > 1)
                        {
                            ...
                            {
                                int subIndexStep = track.SubIndex(subStep + dir * i);
